...
for p = 1:numoftrials
    ...
    for i = 1:nStep_LQG-1
       ...
       A_old_Robust = A_Robust;
        ...
        % Left = LQG model z
        % Right  = Robust model x

        z(1:6,i+1,p)    = x(1:6,i+1,p) + [.06 .15 0 0 0 0]';
        zhat(1:6,i+1,p) = xhat(1:6,i+1,p) + [.06 .15 0 0 0 0]';
        x(7:12,i+1,p)     = z(7:12,i+1,p) - [-.06 .15 0 0 0 0]';
        xhat(7:12,i+1,p)  = zhat(7:12,i+1,p) - [-.06 .15 0 0 0 0]';
        
        eps = x(1:6,i+1,p)- xhat(1:6,i+1,p);
        
        theta_up_R = Ahat_Robust(3,4);
        dzhat_dL = zeros(1,6);
        ...
        eps = z(7:12,i+1,p)- zhat(7:12,i+1,p);
        
        theta_up_L = Ahat_LQG(9,10);
        dzhat_dL = zeros(1,6);
